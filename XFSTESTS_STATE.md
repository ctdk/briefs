...
measured by a fresh full-suite run on the VM.

## Overview

**Latest full-suite run:** 2026-06-29, `./check -s briefs -g auto`
(VM kernel `6.12.94+deb13-amd64`, branch `bh-to-iomap`, module built on the VM
...
| **Pass**      |   341 | executed and matched golden output                        |
| **Not run**   |   440 | skipped by a `_require_*` gate (absent feature or env)    |
| **Fail**      |     2 | `generic/311`, `generic/563` (see below)                  |
| Wedges / oops |     0 | suite completes cleanly, dmesg clean                      |

**Tally reading.** "Failed 2 of 783" — the 783 is the *selected* count, not the
executed count. Executed = 783 − 440 = 343; pass = 343 − 2 = 341.

**Of the 2 failures, neither is an open BrieFS code bug:**

- `generic/311` — pre-existing, reproduces on baseline; dm-flakey/fsync flake.
- `generic/563` — code-fixed (`55023ac`, `sb->s_iflags |= SB_I_CGROUPWB`);
  passes standalone (verified 3/3 + re-verified this session); flakes only
  inside the full suite due to cgroup-writeback setup ordering. The original
  `write=0` bug it closed is gone.

The one remaining *real* open bug — `generic/475` (dm-error crash-replay) —
**passes in-suite**; it only fails as a standalone flake, and is deferred (needs
an on-disk journal-format / sync-model change, explicitly out of scope).
...
### generic/563 — cgroup-aware writeback (code-fixed; in-suite flake)
- **Status:** fail in full suite; **pass standalone** with the current module.
- **Original bug:** `inode_cgwb_enabled()` requires `SB_I_CGROUPWB` on the
  superblock; BrieFS never set it → writeback charged to the fsync submitter
  instead of the dirtier cgroup → cross-cgroup scenario saw `write=0`.
- **Fix:** `55023ac` — one-liner `sb->s_iflags |= SB_I_CGROUPWB` in
  `briefs_fill_super` (every block fs sets it by hand; `mount_bdev`/
  `get_tree_bdev` do not). iomap writeback already calls
  `wbc_init_bio`/`wbc_account_cgroup_owner`; the flag was the only gap.
- **Why it still shows red in-suite:** the full suite runs many tests before
  563; cgroup2/io-controller state left by earlier tests makes 563's `io.stat`
  read return 0 in that context. Standalone (fresh cgroup hierarchy) passes.
  Not a code regression — the `write=0` failure mode is verified absent.
- **Action:** none code-side; the in-suite red is an env-ordering artifact.
...
- **Absent feature** (BrieFS does not implement it): reflink/COW, quota,
  ...
  dedupe, idmapped mounts, O_TMPFILE, FITRIM, chattr/immutable flags, DAX,
  atomic writes, defrag, casefold, setdeleg, fsmap, swapext/startupdate, etc.
  These would need kernel work to close; most are deliberately out of scope.
...
| duperemove utility required (no FIDEDUPERANGE)           | 3 | 559 560 561 |
...
| chattr / FS_IOC_{GET,SET}FLAGS (immutable/append/...)    | 13 | 079 159 160 277 424 507 508 545 553 555 596 607 629 |
...
| inode creation time not supported                         | 1 | 528 |
...
| file_getattr / FSSETXATTR not supported on regular files  | 1 | 772 |
| connectable file handles (export ops lack .get_name)     | 1 | 777 |
| fanotify ioerrors not supported                          | 1 | 791 |
| FSTRIM not supported                                     | 1 | 537 |

> **chattr note:** `generic/472` exercises a swapfile with `chattr` noise on
> stderr ("Operation not supported while reading flags") but **passes** — the
> chattr failure is non-fatal there. The chattr-gated tests above fail at the
> `_require` probe. Implementing `FS_IOC_GETFLAGS`/`FS_IOC_SETFLAGS` (even just
> returning 0 / `FS_IMMUTABLE_FL` plumbing) would clear the chattr row.
...
076  078  080  083  084  085  086  087
088  089  090  091  092  093  094  095
097  098  100  101  102  103  104  106
107  109  112  113  114  117  120  123
124  125  126  127  128  129  130  131
132  133  135  141  169  177  184  192
193  198  204  207  208  209  210  211
212  213  214  215  221  224  225  226
228  236  239  240  241  245  246  247
248  249  250  252  255  256  257  258
263  269  273  274  275  285  286  294
299  300  306  308  309  310  312  313
314  315  316  317  320  321  322  323
325  335  336  337  338  339  340  341
342  343  344  345  346  347  348  354
355  360  361  362  363  364  366  371
376  377  378  390  391  393  394  401
403  405  406  409  410  411  412  416
418  420  422  423  426  427  428  430
431  432  433  434  436  437  438  439
441  443  445  446  448  450  451  452
453  454  460  464  465  466  467  471
472  475  476  477  478  479  480  481
483  484  486  488  489  490  491  494
495  496  498  502  504  510  512  519
520  523  524  525  526  527  532  533
534  535  538  539  547  551  552  554
557  558  564  567  568  569  571  585
586  589  590  591  597  598  604  609
611  615  616  617  618  619  620  626
627  631  632  633  634  636  637  638
...
> It passes in-suite (so not counted as a failure above) but flakes when run
> standalone; deferred — requires an on-disk journal-format / sync-model
> change, explicitly out of scope (no forced-block / seed-pool / ordered-write
> approaches).
...
- `563` re-verified standalone (passes) to confirm the in-suite red is
  env-ordering, not a code regression; that standalone re-run cleaned the
  `.out.bad`, so the in-suite mismatch detail is no longer on disk.